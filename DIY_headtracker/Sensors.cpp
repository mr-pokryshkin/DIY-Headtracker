...
#define ACC_SENS 4096         // Sensitivity. 13 bit adc, +/- 16 g. Calculated as: (2^13)/(16*2)
...
//    
//    ReadFromI2C(BMA180, DATA, 6);
    accRaw[0] =((( (int)sensorBuffer[0] | (int)sensorBuffer[1] << 8) >> 2) + offx )*-1;
    accRaw[1] =((( (int)sensorBuffer[2] | (int)sensorBuffer[3] << 8 ) >> 2) + offy)*-1;
    accRaw[2] = (( (int)sensorBuffer[4] |(int) sensorBuffer[5] << 8 ) >> 2) + offz; 
...
//
{
  byte temp1;
  //
  WriteToI2C(BMA180, RESET, 0xB6);
  //wake up mode
  WriteToI2C(BMA180, PWR, 0x10);
  // low pass filter,
  ReadFromI2C(BMA180, BW, 1);
  temp1 = sensorBuffer[0] & 0x0F;
  WriteToI2C(BMA180, BW, temp1);
  // range +/- 2g
  ReadFromI2C(BMA180, RANGE, 1);
  temp1 = (sensorBuffer[0] & 0xF1) | 0x04;
  WriteToI2C(BMA180, RANGE, temp1);
}
//--------------------------------------------------------------------------------------
// Func: InitSensors
